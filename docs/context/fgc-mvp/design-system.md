...
Extraído de `fgc-app-google-ai-studio/` em **19/09/2026**. Referência para reutilização futura da identidade visual e dos padrões de interface do protótipo.

## 1. Escopo e confiabilidade

Este documento descreve o **código existente**, não um redesign nem uma especificação de funcionalidades de produção. Foram lidos todos os arquivos de `src/`, as configurações de entrada, o manifesto e o lockfile. A referência Git é `c5eb3ea4ec048314bfd52076a867b040e89b8abb`; na análise, `package-lock.json` estava presente localmente, mas não versionado. Nenhum arquivo do protótipo foi alterado.
...
Fonte canônica: `fgc-app-google-ai-studio/src/index.css`, bloco `@theme`. As 14 cores são tokens reais; os nomes abaixo podem formar classes como `bg-surface`, `text-primary` e `border-border`.
...
| `--color-success` | `#22C55E` | Concluído, respondido, progresso favorável e vitórias |
| `--color-warning` | `#F59E0B` | Em andamento, pendências, segunda entrevista e espera |
| `--color-danger` | `#EF4444` | Urgência, erro, ocupado, derrotas e indicador de transmissão |
...
- Aparência desabilitada de “Finalized”: `bg-primary/20 text-primary/50 cursor-not-allowed`.
- Segunda entrevista: faixa `bg-warning/70` e texto `text-warning/80`.
...
| Classificação da equipe | Conteúdo vertical e centralizado | Horizontal, título à esquerda e divisor visível |
...
| Ação secundária | Borda neutra, fundo herdado, hover `bg-surface-container-low`, altura de 40 px | “Edit Score” |
...
| Campo de acesso | `h-14 rounded-xl text-center text-2xl tracking-[0.5em]`, senha com limite de 6 caracteres | Login |
...
| Botão flutuante | `fixed bottom-24 right-6`, pílula escura, `px-6 py-4`, ícone e sinal pulsante | Agenda |
...
- **Desabilitado:** “Finalized” possui apenas aparência desabilitada; falta o atributo `disabled`.
- **Loading, vazio, sucesso de envio e falha de rede:** sem estados implementados. Listas exibem dados fixos.

## 8. Semântica visual dos estados

| Contexto | Estado | Representação observada |
| --- | --- | --- |
| Entrevista | `Completed` | Verde; ações “Edit Score” e “Finalized” |
| Entrevista | `In Progress` | Âmbar, faixa lateral e `ring-2 ring-warning`; ação “Mark Evaluated” |
| Entrevista | `Needs Second Interview` | Âmbar com opacidades reduzidas; ação “Start Interview” |
| Entrevista | `Not Started` | Cinza `outline`; ação “Start Interview” |
| Mensagem | `Responded` | Verde com ponto e fundo a 10% |
| Mensagem | `Acknowledged` | Azul secundário com ponto e fundo a 10% |
| Mensagem | `Pending` | Âmbar com ponto e fundo a 10% |
| Mensagem | `Busy` | Existe no tipo, mas não nos dados exibidos; o fallback atual do pager o pintaria de âmbar |
| Resposta do mentor | “On the way” | Botão verde preenchido |
| Resposta do mentor | “Busy (in match)” | Botão vermelho preenchido |
| Resposta do mentor | “Available in 5 mins” | Contorno e texto âmbar |
| Resposta do mentor | “Request Details” | Contorno e texto `primary` |
| Mapa | Avaliado / ativo / não visitado | Marcadores verde / âmbar pulsante / cinza |

Há inconsistências que não devem virar regras de domínio: a equipe USA está como segunda entrevista na lista, mas seu marcador é cinza no mapa; `Busy` não tem variante específica no pager; vermelho também indica transmissão ao vivo e derrota. Os dados e marcadores são independentes e fixos. **Recomendado:** definir o vínculo entre estado de domínio e aparência ao integrar dados reais, sem inferir regras a partir desses exemplos.
...
| Indicador ao vivo | `animate-ping`, padrão instalado de 1 s, repetição infinita |
...
## 11. Composição das telas e limites funcionais

| Tela | Composição | Comportamento existente |
| --- | --- | --- |
| Login | Marca, foto, card de acesso, ajuda, rodapé | Código local escolhe um dos três perfis; erro para código desconhecido |
| Advisor | Progresso, três métricas, dois alertas, submissões recentes | Conteúdo demonstrativo; botões sem ações conectadas |
| Painel do juiz | Busca, resumo do painel, equipes, mapa | Lista e estados fixos; busca e ações não alteram os dados |
| Mentor | Alerta urgente, quatro respostas, atividade recente | Respostas sem handlers; alerta e histórico fixos |
| Pager | Busca de equipe, quatro presets, envio e histórico | Sem seleção, envio ou busca implementados |
| Agenda | Busca, tabela de partidas, classificação, botão ao vivo | Busca preenchida por padrão; sem filtro ou destino de transmissão |

O shell alterna abas com estado React, abre/fecha o menu, fecha-o pelo backdrop e encerra a sessão local. Não há roteamento por URL. O dashboard depende do perfil: advisor → `JudgeAdvisorDashboard`, judge → `JudgePanelScreen`, mentor → `MentorPager`. A aba Map também renderiza `JudgePanelScreen`, incluindo a lista; não existe tela exclusiva de mapa.

**Uso futuro:** preservar a composição visual quando aplicável, mas não tratar disponibilidade de abas, textos fixos, valores de métricas ou demonstração de login como especificações de autorização, autenticação ou regras de negócio.
...
4. “Finalized” não está semanticamente desabilitado. `MoreVertical` tem `cursor-pointer`, mas é um SVG sem botão, handler ou interação de teclado.
...
## 13. Guia para implementação posterior

Esta seção propõe uma organização; não descreve componentes já disponíveis.
...
4. **Estados:** conectar variantes aos tipos de domínio validados; tratar `Busy`, segunda entrevista, loading, vazio, erro e disabled explicitamente.
...
6. **Verificação:** comparar as seis telas em larguras móveis e desktop, com teclado, foco, contraste, movimento reduzido e dados reais. Essa validação não foi realizada nesta extração.
