  ...
  function wrap(client: typeof publicClient, service: boolean) {
    return {
      async rpc(name: string, args: Record<string, unknown> = {}): Promise<unknown> {
        if (service && !serviceOperations.has(name))
          throw new DatabaseFailure('FORBIDDEN', 'FORBIDDEN')
        const result = await client.schema('api').rpc(name, args)
        if (result.error)
          throw new DatabaseFailure(result.error.code, result.error.message)
        return result.data
      },
    }
  }
  return {
    ...
    async verify(token: string): Promise<{ id: string }> {
      ...
      if (result.error || !result.data.user)
        throw new DatabaseFailure('UNAUTHENTICATED', 'UNAUTHENTICATED')
      return { id: result.data.user.id }
    },
    async ready(): Promise<boolean> {
      try {
        const result = await privileged.schema('api').rpc('health_check')
