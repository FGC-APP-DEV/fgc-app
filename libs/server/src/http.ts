...
export function createApi(config: ApiConfig) {
  ...
  app.use(
    cors({
      origin: (origin, done) =>
        done(null, !origin || config.allowedOrigins.includes(origin)),
      credentials: true,
    }),
  )
  app.use('/api/v1/imports/preview', express.json({ limit: '8mb' }))
  app.use(express.json({ limit: '256kb' }))
  const send = (res: Response, data: unknown, nextCursor?: string) =>
    res.json({
      data,
      meta: { requestId: res.locals.requestId, ...(nextCursor ? { nextCursor } : {}) },
    })
  ...
  const sign = (value: string) =>
    createHmac('sha256', config.mentorSecret).update(value).digest('hex')
  async function staff(
    ...
    const user = (await rpc.rpc('me')) as c.User | null
  ...
  function mentor(req: Request, mutate = false): string {
    const authorization = req.get('Authorization')
    const cookie = cookies(req)[cookieName]
    if (authorization && cookie) throw new DomainError('UNAUTHENTICATED')
    if (authorization && !authorization.startsWith('Mentor '))
      throw new DomainError('UNAUTHENTICATED')
    ...
    if (cookie && mutate) {
      ...
      if (
        !config.allowedOrigins.includes(req.get('Origin') ?? '') ||
  ...
  function pagination(req: Request) {
    const query = c.listQuery.parse(req.query)
    let after: string | null = null
    if (query.cursor) {
      try {
        after = c.uuid.parse(Buffer.from(query.cursor, 'base64url').toString('utf8'))
      } catch {
  ...
  app.get(
    '/health/ready',
    wrap(async (_, res) => {
      const ready = await config.gateway.ready()
  ...
  app.post(
    ...
    wrap(async (req, res) => {
      const { rpc } = await staff(req, 'admin')
      const input = c.importPreviewInput.parse(req.body)
      const preview = await readImport(input)
      const key = c.uuid.parse(req.get('Idempotency-Key'))
      const receipt = (await rpc.rpc('import_preview', {
        p_input: preview,
        p_key: key,
      })) as c.Receipt
      return send(res, {
        ...((await rpc.rpc('import_read', {
          p_id: receipt.entityId,
        })) as c.ImportPreview),
  ...
  app.get(
    ...
    wrap(async (req, res) => {
      ...
      return send(
        res,
        await rpc.rpc('import_read', { p_id: c.uuid.parse(req.params.id) }),
      )
    }),
  )
  app.post(
    ...
    wrap(async (req, res) => {
      const { rpc } = await staff(req, 'admin')
      const id = c.uuid.parse(req.params.id)
      const input = c.versionInput.parse(req.body)
      const key = c.uuid.parse(req.get('Idempotency-Key'))
      ...
      for (const row of preview.rows.filter((r) => r.status === 'ready')) {
        try {
          await rpc.rpc('import_row_commit', {
            p_input: {
              previewId: id,
              row: row.row,
              expectedVersion: input.expectedVersion,
            },
            p_key: deriveKey(key, String(row.row)),
          })
        } catch (error) {
      ...
      return send(res, {
        ...((await rpc.rpc('import_read', { p_id: id })) as c.ImportPreview),
        errors,
      })
    }),
  )
  app.post(
    ...
    wrap(async (req, res) => {
      const { rpc } = await staff(req, 'admin')
      const input = c.accessInput.parse(req.body)
      const key = c.uuid.parse(req.get('Idempotency-Key'))
      const results = []
      for (const email of input.emails) {
        try {
          results.push({
            email,
            receipt: await rpc.rpc('access_grant', {
              p_input: {
                email,
                roles: input.roles,
                mode: input.mode,
              ...
              p_key: deriveKey(key, email),
            }),
          })
        } catch (error) {
          results.push({
            email,
            error: error instanceof DomainError ? error.code : 'DEPENDENCY_UNAVAILABLE',
          })
        }
      }
  ...
  for (const route of reads)
    app.get(
      `/api/v1${route.path}`,
      wrap(async (req, res) => {
        const { rpc } = await staff(req, route.capability)
        ...
        return send(
          ...
          route.paginated
            ? nextCursor(
                route.rpc === 'tracker' ? (data as c.Tracker).teams : data,
                query.limit,
              )
            : undefined,
        )
  ...
  for (const route of commands)
    app[route.method](
      ...
      wrap(async (req, res) => {
        const { rpc } = await staff(req, route.capability)
        const input = route.schema.parse(req.body) as Record<string, unknown>
        const key = c.uuid.parse(req.get('Idempotency-Key'))
        for (const [param, field] of Object.entries(route.params ?? {}))
          input[field] =
            param === 'type'
              ? c.flagType.parse(req.params[param])
              : c.uuid.parse(req.params[param])
        if (route.rpc === 'flag_put' && input.type === 'other' && !input.reason)
          throw new DomainError('VALIDATION_ERROR')
        if (route.rpc === 'page_create') {
          await staff(req, input.sourceArea === 'judges' ? 'judging' : 'filming')
        }
        return send(res, await rpc.rpc(route.rpc, { p_input: input, p_key: key }))
      }),
    )
  app.post(
    '/api/v1/admin/mentor-codes',
    wrap(async (req, res) => {
      const { rpc } = await staff(req, 'admin')
      const input = c.mentorCodeInput.parse(req.body)
      const key = c.uuid.parse(req.get('Idempotency-Key'))
      const code = randomBytes(6).toString('hex').toUpperCase()
      const receipt = await rpc.rpc('mentor_code_issue', {
        p_input: { ...input, digest: sign(code) },
        p_key: key,
      })
  ...
  app.post(
    '/api/v1/mentor/redeem',
    wrap(async (req, res) => {
      if (req.get('Authorization') || cookies(req)[cookieName])
        throw new DomainError('STATE_CONFLICT')
      const input = c.redeemInput.parse(req.body)
      const digest = sign(input.code.replace(/[\s-]/g, '').toUpperCase())
      if (
        input.platform === 'web' &&
        !config.allowedOrigins.includes(req.get('Origin') ?? '')
      )
        throw new DomainError('FORBIDDEN')
      const allowed = await config.gateway.service.rpc('mentor_rate_limit', {
        p_ip_hash: sign(req.ip ?? 'unknown'),
        p_installation_id: input.installationId,
        p_digest: digest,
      })
      if (allowed !== true) throw new DomainError('RATE_LIMITED')
      const token = randomBytes(32).toString('hex')
      await config.gateway.service.rpc('mentor_redeem', {
        p_digest: digest,
        p_token_hash: hash(token),
      })
      const data = (await config.gateway.service.rpc('mentor_me', {
        p_token_hash: hash(token),
      })) as c.Mentor
      ...
      return send(res, {
        ...data,
        ...(input.platform === 'mobile' ? { sessionToken: token } : {}),
      })
    }),
  )
  app.get(
    ...
    wrap(async (req, res) => {
      ...
      if (!origin && req.get('Sec-Fetch-Site') === 'same-origin') {
        try {
          origin = new URL(req.get('Referer') ?? '').origin
        } catch {
          /* Unproven origin is rejected. */
        }
      }
      if (!origin || !config.allowedOrigins.includes(origin))
        throw new DomainError('FORBIDDEN')
      return send(res, { csrfToken: sign(mentor(req)) })
    }),
  )
  for (const resource of ['me', 'filming', 'pages'])
    app.get(
      `/api/v1/mentor/${resource}`,
      wrap(async (req, res) => {
        ...
        return send(
          res,
          data,
          resource === 'pages' ? nextCursor(data, query.limit) : undefined,
        )
      }),
    )
  app.post(
    '/api/v1/mentor/pages/:id/respond',
    wrap(async (req, res) =>
      send(
        res,
        await config.gateway.service.rpc('mentor_respond', {
          p_token_hash: hash(mentor(req, true)),
          p_input: {
            ...c.responseInput.parse(req.body),
            pageId: c.uuid.parse(req.params.id),
          },
          p_key: c.uuid.parse(req.get('Idempotency-Key')),
        }),
      ),
    ),
  )
  app.put(
    '/api/v1/mentor/device',
    wrap(async (req, res) =>
      send(
        res,
        await config.gateway.service.rpc('mentor_device_put', {
          p_token_hash: hash(mentor(req, true)),
          p_input: c.deviceInput.parse(req.body),
        }),
      ),
    ),
  )
  app.delete(
    ...
    wrap(async (req, res) =>
      send(
        res,
        await config.gateway.service.rpc('mentor_device_delete', {
          p_token_hash: hash(mentor(req, true)),
          p_installation_id: c.uuid.parse(req.body.installationId),
        }),
      ),
    ),
  )
  app.post(
    '/api/v1/mentor/logout',
    wrap(async (req, res) => {
      await config.gateway.service.rpc('mentor_logout', {
        p_token_hash: hash(mentor(req, true)),
      })
      ...
      return send(res, { signedOut: true })
    }),
  )
  app.use((_, res) =>
    res.status(404).json({
