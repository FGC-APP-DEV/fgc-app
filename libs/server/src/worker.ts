  ...
  router.post('/tick', async (req, res) => {
    ...
    try {
      ...
      await Promise.all(ids.map(async id => {
        const delivery = await service.rpc('delivery_authorize', { p_id: id }) as { deliveryId: string; token: string; title: string; body: string } | null;
        if (!delivery) { await service.rpc('delivery_finish', { p_id: id, p_accepted: false, p_invalid_token: false }); return; }
        ...
        await service.rpc('delivery_finish', { p_id: id, p_accepted: accepted, p_invalid_token: invalid });
      }));
