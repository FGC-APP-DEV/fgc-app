...
it('preserves leading zeros and groups duplicates without choosing conflicting rows', async () => {
  const rows = [
    { id: '001', name: 'Alpha', country: 'BR' },
    { id: '001', name: 'Alpha', country: 'BR' },
    { id: '002', name: 'Beta', country: 'US' },
    { id: '002', name: 'Other', country: 'US' },
    { id: '003', name: '', country: 'BR' },
  ]
  const preview = await imports.readImport({
    fileName: 'teams.json',
  ...
  expect(preview.rows.map((r) => [r.officialId, r.status])).toEqual([
    ['001', 'ready'],
    ['001', 'duplicate'],
    ['002', 'conflict'],
    ['002', 'conflict'],
