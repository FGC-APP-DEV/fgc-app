...
export const profileInput = versionInput.extend({
...
export const transferInput = versionInput.extend({
  sourcePanelId: uuid,
  targetPanelId: uuid,
...
export const observationInput = versionInput.extend({
  panelId: uuid,
...
export const verifyInput = z
  .object({
    attemptId: uuid,
    emailCode: z.string().regex(/^\d{6,10}$/),
    codeVerifier: z.string().min(43).max(128),
  })
  .strict()
export const confirmLinkInput = z
  .object({ attemptId: uuid, tokenHash: z.string().min(20).max(300) })
  .strict()
...
export const importPreviewInput = z
  .object({
    ...
    countries: z.record(z.string().regex(/^[A-Z]{2}$/)).default({}),
  })
  .strict()
export const listQuery = z
  .object({
    ...
    sourceArea: pageSource.optional(),
  })
  .strict()
...
export interface Receipt {
  ...
  outcome: 'created' | 'updated' | 'deleted' | 'noop'
  committedAt: string
}
...
export interface Participation {
  ...
  participationStatus: 'active' | 'withdrawn'
  hasHistory: boolean
  version: number
  flags: { type: 'absent' | 'online' | 'other'; reason: string; version: number }[]
...
export interface TrackerTeam extends Team {
  shots: Shot[]
}
...
export interface Page {
  ...
  createdAt: string
  response: (typeof mentorResponses)[number] | null
...
export interface ImportRow {
  ...
  name: string
  country: string
  status: 'ready' | 'invalid' | 'duplicate' | 'conflict' | 'existing' | 'imported'
  errors: string[]
}
...
export const pagerPresets: Record<PageSource, readonly string[]> = {
  filming: [
    'Please come to the filming booth for a quick shoot.',
    "We'd like to interview your team — please come to the production area.",
    'Time for your Step & Repeat photo — please come to the backdrop.',
    'Robot reveal shoot in ~5 min — please bring your robot to the filming area.',
  ],
  judges: [
    'Please come to the judging area.',
    "We're ready for your interview — please come to the judging room.",
    'Technical inspection required — please come to the judging area.',
    'Please return to your pit immediately.',
  ],
}
