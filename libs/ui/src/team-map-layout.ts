...
export function layoutTeams(
  ...
  sorted.forEach((team, index) => {
    const hasCoordinate =
      Number.isFinite(team.pitX) && Number.isFinite(team.pitY);
    let gridX: number;
    let gridY: number;
    ...
    if (hasCoordinate) {
      ...
      const block = Math.min(
        geometry.blocks - 1,
        Math.floor(index / perBlock),
      );
      const withinBlock = index - block * perBlock;
      gridX =
        block *
          ((blockWidth + geometry.blockGap) /
            (geometry.cellWidth + geometry.gap)) +
    ...
    out.set(team.id, {
      cx:
        40 +
        gridX * (geometry.cellWidth + geometry.gap) +
...
export function clampTeamMapZoom(zoom: number): number {
  return Math.min(
    TEAM_MAP_GEOMETRY.maxZoom,
    Math.max(TEAM_MAP_GEOMETRY.minZoom, zoom),
  );
}
